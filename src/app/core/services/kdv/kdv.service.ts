...
import { LangKey, AppMode } from 'src/app/modules/common-core/models';
...
export class KdvService {
  ...
  constructor(
    private kdvApiService: KdvElementsService,
  ...
  async updateKdvElementsForLanguage(appMode: AppMode, language: LangKey, cancel?: Promise<void>) {
    ...
    try {
      const kdvElements = await toPromiseWithCancel(this.kdvApiService.KdvElementsGetKdvs({ langkey: language }), cancel, 20000);
  ...
  private getKdvElementsFromDbAsStream(appMode: AppMode, langKey: LangKey) {
    return new NSqlFullUpdateObservable<KdvElementsResponseDto[]>(
      NanoSql.getInstance(NanoSql.TABLES.KDV_ELEMENTS.name, appMode).query('select').where(['langKey', '=', langKey]).listen()
