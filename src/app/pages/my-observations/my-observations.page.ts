...
const LIST_HEADERS = {
  draft: {
    header: 'REGISTRATION.DRAFTS',
    subtitle: 'REGISTRATION.DRAFTS_DESCRIPTION'
  },
  sync: {
    header: 'REGISTRATION.SAVED_ON_PHONE',
    subtitle: 'REGISTRATION.SAVED_DESCRIPTION'
  },
  sent: {
    header: 'MY_OBSERVATIONS.MY_SENT_OBSERVATIONS',
    subtitle: 'MY_OBSERVATIONS.SENT_SUBTITLE'
  }
};
...
export class MyObservationsPage implements OnInit, OnDestroy {
  ...
  private initRegistrationSubscription() {
    this.registrationSubscription = this.getVirtualScrollItemsObservable().subscribe(
      (result) => {
    ...
    return this.getSyncItemsObservable().pipe(
      switchMap((syncItems) =>
        combineLatest([
          ...
          this.getMyRegistrationsObservable(0)
        ])
      ),
      map(([syncIntems, drafts, registrations]) => [
        ...syncIntems,
        ...drafts,
        ...registrations
      ])
  ...
  private getMyRegistrationsObservable(
    pageNumber: number
  ): Observable<MyVirtualScrollItem[]> {
  ...
  private getDraftObservable(): Observable<MyVirtualScrollItem[]> {
    return this.registrationService.drafts$.pipe(
      map((val) =>
        val.map((item) => ({ type: <const>'draft', id: item.id, item }))
      )
    );
  }
  ...
  private getSyncItemsObservable(): Observable<MyVirtualScrollItem[]> {
    return this.registrationService.getRegistrationsToSync().pipe(
      distinctUntilChanged<IRegistration[], string>(
        (a, b) => a.localeCompare(b) === 0,
        (keySelector) => this.getDistinctSyncItemList(keySelector)
      ),
      map((val) =>
        val.map((item) => ({ type: <const>'sync', id: item.id, item }))
      )
    );
  }
  ...
  getDistinctRegistrationList(arr: RegistrationViewModel[]) {
    return arr
      ? arr.map((reg) => `${reg.RegID}-${reg.DtChangeTime}`).join('#')
      : '';
  }
  ...
  async loadMoreData(event: IPageInfo) {
    ...
    const currentLength = this.virtualItems.filter((x) => x.type === 'sent')
      .length;
    const pageNumber = Math.floor(currentLength / numberOfItemsToFetch);
    this.getMyRegistrationsObservable(pageNumber)
      .pipe(
        take(1),
        finalize(() => {
      ...
      .subscribe(
        (nextPage) => (this.virtualItems = this.virtualItems.concat(nextPage))
      );
  }
