...
//   providedIn: 'root'
// })
// export class RegistrationRepositoryService
//   extends NgDestoryBase
//   implements OnReset {
//   public readonly registrations$: Observable<IRegistration[]>;
//   private readonly inMemoryRegistrations = new BehaviorSubject<{
//     [appMode: string]: IRegistration[];
//   }>({});
...
//           'Registrations from repository is',
//           DEBUG_TAG,
//           registrations
//         )
//       ),
...
//     const mergedResult = this.mergeRegistrationsBasedOnId(
//       [reg],
...
//       .where(['id', '=', id])
//       .exec();
//     const newInMemoryRegistrations = this.getCurrentInMemoryRegistrations(
//       appMode
...
//         switchMap((appMode) =>
//           combineLatest([
//             this.getInMemoryRegistrationsForAppMode(appMode).pipe(take(1)),
//             this.getRegistrationsFromDb(appMode),
//             of(appMode)
//           ])
//         ),
//         concatMap(([imr, dbRegistrations, appMode]) =>
//           of(this.mergeRegistrationsBasedOnId(imr, dbRegistrations)).pipe(
//             map((result) => ({ appMode, result }))
//           )
//         ),
...
//       .subscribe((result) => {
//         this.loggingService.debug(
//           'Merged existing registrations from database.',
//           DEBUG_TAG,
//           result
//         );
...
//             debounceTime(200),
//             map((registrations) => ({ appMode, registrations }))
//           )
//         ),
//         tap((result) =>
//           this.loggingService.debug(
//             'InMemory registrations changed. Saving to db: ',
//             DEBUG_TAG,
//             result
//           )
//         ),
//         concatMap((result) =>
//           this.saveRegistrationsToDb(result.appMode, result.registrations)
//         ),
//         takeUntil(this.ngDestroy$)
//       )
//       .subscribe();
...
//   private saveInMemoryRegistrations(
//     appMode: AppMode,
...
//     return from(
//       NanoSql.getInstance(NanoSql.TABLES.REGISTRATION.name, appMode)
//         .query('select')
//         .exec() as Promise<IRegistration[]>
//     );
//   }

//   private mergeRegistrationsBasedOnId(
//     registrationsToKeep: IRegistration[],
//     registrationsToMerge: IRegistration[]
//   ) {
//     registrationsToKeep = registrationsToKeep || [];
//     registrationsToMerge = registrationsToMerge || [];
//     return [
//       ...registrationsToKeep,
//       ...registrationsToMerge.filter(
//         (newReg) => !registrationsToKeep.find((cr) => cr.id === newReg.id)
//       )
//     ];
//   }

//   private getInMemoryRegistrationsForAppMode(
//     appMode: AppMode
//   ): Observable<IRegistration[]> {
//     this.loggingService.debug(
//       `Get inMemory registrations for app mode: ${appMode}`,
//       DEBUG_TAG
//     );
//     return this.inMemoryRegistrations.pipe(
//       map((inMemoryReg) => inMemoryReg[appMode] || [])
//     );
//   }

//   private saveRegistrationsToDb(
//     appMode: AppMode,
//     registrations: IRegistration[]
//   ): Observable<IRegistration[]> {
//     return from(
//       NanoSql.getInstance(NanoSql.TABLES.REGISTRATION.name, appMode)
//         .query('upsert', registrations)
//         .exec() as Promise<IRegistration[]>
//     ).pipe(
//       tap((result) => {
//         this.loggingService.debug(
//           'Result from save to db is',
//           DEBUG_TAG,
//           result
//         );
//       }),
...
//           'Could not save registrations to offline db',
//           err,
//           LogLevel.Warning,
//           DEBUG_TAG
//         );
