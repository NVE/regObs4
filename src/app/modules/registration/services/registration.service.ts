...
export class RegistrationService {
  ...
  private async cleanupImages(
    request: RegobsApiModels.CreateRegistrationRequestDto
  ) {
    try {
      if (!request) {
        return;
      ...
      if (request.Picture && request.Picture.length > 0) {
        request.Picture = await asyncFilter(request.Picture, (p) =>
      ...
      if (request.DamageObs && request.DamageObs.length > 0) {
        request.DamageObs = await Promise.all(
          request.DamageObs.map(async (dto) => ({
            ...dto,
            Pictures: await asyncFilter(dto.Pictures, (p) =>
              this.isValidPicture(p)
            )
          }))
        );
  ...
  getRegistrationsToSync() {
    return this.registrations$.pipe(
      map((items) => items.filter((x) => x.status === RegistrationStatus.Sync))
    );
  }
