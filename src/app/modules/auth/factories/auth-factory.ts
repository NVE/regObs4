...
export const authFactory = (platform: Platform, ngZone: NgZone,
  requestor: Requestor, browser: Browser, storage: StorageBackend, userSettingService: UserSettingService): AuthService => {
  ...
  userSettingService.appMode$.subscribe((appMode: AppMode) => {
    ...
    if (!platform.is('cordova')) {
      ...
      authService.authConfig.end_session_redirect_url = window.location.origin + '/auth/callback';
    }
  });
  return authService;
};
